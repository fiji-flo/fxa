...
export const Page2faReplaceRecoveryCodes = (_: RouteComponentProps) => {
  ...
  const [subtitle, setSubtitle] = useState<string>(l10n.getString('tfa-replace-code-1-2', null, 'Step 1 of 2'));
  const [recoveryCodes, setRecoveryCodes] = useState<string[]>([]);
  const [recoveryCodesAcknowledged, setRecoveryCodesAcknowledged] = useState<boolean>(false);
  ...
  const activateStep = (step:number) => {
    ...
    {
      case 1:
        setSubtitle(l10n.getString('tfa-replace-code-1-2', null, 'Step 1 of 2'));
        setRecoveryCodesAcknowledged(false);
        break;

      case 2:
        setSubtitle(l10n.getString('tfa-replace-code-2-2', null, 'Step 2 of 2'));
  ...
  return (
    ...
    >
      ...
      {recoveryCodesAcknowledged&& (
        <RecoveryCodeCheck recoveryCodes={recoveryCodes} goHome={goHome} onRecoveryCodeSubmit={onRecoveryCodeSubmit} cancellable={false} />
      )}
    </FlowContainer>
  );
};
...
// TODO: Reuse RecoveryCodeCheck in PageTwosetpAuthentication
  ...
  return (
    <form onSubmit={recoveryCodeForm.handleSubmit(onSubmit)}>
      ...
      <div className="flex justify-center mb-4 mx-auto max-w-64">
        {cancellable && <Localized id="tfa-button-cancel">
          <button
            type="button"
